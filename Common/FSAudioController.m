        ...
        _audioStream.onCompletion = ^() {
            ...
            if ([weakSelf.audioController.playlistItems count] > 0) {
                ...
                if (weakSelf.audioController.currentPlaylistItemIndex + 1 < [weakSelf.audioController.playlistItems count]) {
                    ...
                    if (weakSelf.audioController.onRecordTrackAvailable) {
                        weakSelf.audioController.audioStream.onRecordTrackAvailable = weakSelf.audioController.onRecordTrackAvailable;
                    }
                    if (weakSelf.audioController.onRecordError) {
                        weakSelf.audioController.audioStream.onRecordError = weakSelf.audioController.onRecordError;
                    }
...
- (void)play
{
    ...
    if (self.onRecordTrackAvailable) {
        self.audioStream.onRecordTrackAvailable = self.onRecordTrackAvailable;
    }
    if (self.onRecordError) {
        self.audioStream.onRecordError = self.onRecordError;
    }
...
- (void (^)(NSString *recordDirectory, NSString *recordFile, NSDictionary *metadata, BOOL finish))onRecordTrackAvailable
{
    return _onRecordTrackAvailableBlock;
}

- (void)setOnRecordTrackAvailable:(void (^)(NSString *, NSString *, NSDictionary *, BOOL))onRecordTrackAvailable
{
    _onRecordTrackAvailableBlock = onRecordTrackAvailable;
    
    self.audioStream.onRecordTrackAvailable = _onRecordTrackAvailableBlock;
}

- (void (^)(NSError *))onRecordError
{
    return _onRecordErrorBlock;
}

- (void)setOnRecordError:(void (^)(NSError *))onRecordError
{
    _onRecordErrorBlock = onRecordError;
    
    self.audioStream.onRecordError = _onRecordErrorBlock;
}
